...
def launch_container(
    ...
    if log_cli:
        ...
        volume_parts = [
            f"-v {v.host_path}:{v.container_path}:{v.mode}" for v in volume_mounts or []
        ]
        env_parts = [f"-e {key}={value}" for key, value in (environment or {}).items()]
...
def get_docker_logs(
    container_id: str, client: docker.DockerClient, log_cli: bool = False
) -> str:
    """
...
def cleanup_docker_containers(
    container_names: List[str], client: docker.DockerClient, log_cli: bool = False
) -> None:
    """
...
def create_docker_network(
    network_name: str, client: docker.DockerClient, log_cli: bool = False
) -> bool:
    """
